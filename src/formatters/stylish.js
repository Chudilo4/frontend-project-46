...
const formatValue = (val, nestingLevel) => {
  if (typeof val === 'object' && val !== null) {
    ...
    const result = objectKeys
      .map(
        (key) => `${idents}${key}: ${formatValue(val[key], nestingLevel + 1)}\n`,
      )
      .join('');
    ...
    switch (currDiff.keyStatus) {
      case KEY_UNCHANGED:
        return formatLine(
          ...
          ' ',
          currDiff.key,
          formatValue(currDiff.first, nestingLevel + 1),
        );
      case KEY_ADDED:
        return formatLine(
          idents,
          '+',
          currDiff.key,
          formatValue(currDiff.second, nestingLevel + 1),
        );
      case KEY_DELETED:
        return formatLine(
          idents,
      ...
      case KEY_NESTED_DIFF:
        return formatLine(
          ...
          ' ',
          currDiff.key,
          formatObjectDiff(currDiff.nestedDiff, nestingLevel + 1),
        );
      default:
